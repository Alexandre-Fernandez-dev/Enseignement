...
\tcbox[tikznode]{
Chapitre 5 \\ Arithmétique PGCD et applications
}
}
...
Grand Commun Diviseur de a et b}, noté $\operatorname{PGCD}(a ; b)$.
\end{definition}
...
\begin{itemize}[noitemsep,topsep=0pt]
  \item $\operatorname{PGCD}(a ; b) \geqslant 1$;
  \item $a \mid b \Longleftrightarrow \operatorname{PGCD}(a ; b)=a$;
\end{itemize}
\end{proposition}
...
\begin{itemize}[noitemsep,topsep=0pt]
  ...
  \item $\rightarrow$ Si $r_{0}=0$, on pose $r_{1}=0$;\\
$\rightarrow$ Sinon $r_{1}$ est le reste de la division euclidienne de $b$ par $r_{0}$.
  \item pour tout $n \geqslant 1$ :\\
$\rightarrow$ Si $r_{n}=0$, on pose $r_{n+1}=0$.\\
$\rightarrow$ Sinon, $r_{n+1}$ est le reste de la division euclidienne de $r_{n-1}$ par $r_{n}$.
\end{itemize}
...
$$
d \mid a \text { et } d|b \Longleftrightarrow d| \operatorname{PGCD}(a ; b)
$$
\end{proposition}
...
$$
\operatorname{PGCD}(k a ; k b)=k \times \operatorname{PGCD}(a ; b)
$$
\end{proposition}
...
On dit que $a$ et $b$ sont premiers entre eux lorsque $\operatorname{PGCD}(a ; b)=1$.
\end{definition}

\begin{proposition}{}{}
Soient $a, b \in \mathbb{N}^{*}$ et avec $d=\operatorname{PGCD}(a ; b)$.\\
Il existe des entiers premiers entre eux $a^{\prime}$ et $b^{\prime}$ tels que $\left\{\begin{array}{l}a=d a^{\prime} \\ b=d b^{\prime}\end{array}\right.$
\end{proposition}
...
Si $a \mid b c$ et si $a$ et $b$ sont premiers entre eux, alors $a \mid c$.
\end{proposition}
...
Soient $a, b, c \in \mathbb{Z}^{*}$ tels que $b|a, c| a$ et tels que $\operatorname{PGCD}(b ; c)=1$, alors $b c \mid a$.
\end{proposition}
...
& a \equiv 0[b] \\
& a \equiv 0[c] \\
& \operatorname{PGCD}(b ; c)=1
\end{array} \Longrightarrow a \equiv 0[b c] .
\right.
$$
\end{proposition}
...
\begin{itemize}[noitemsep,topsep=0pt]
  \item $a$ est pair $\Longleftrightarrow a^{2}$ est pair.
  \item $a$ est impair $\Longleftrightarrow a^{2}$ est impair.
\end{itemize}
\end{proposition}
...
\begin{proposition}{}{}
Soint $n \geqslant 2$. Alors $a \in \mathbb{N}$ est inversible modulo $n$ si, et seulement si, $\operatorname{PGCD}(a ; n)=1$.
\end{proposition}
...
L'équation $a x+b y=k$ (d'inconnues $x$ et $y$ ) admet des solutions entières si, et seulement si, $\operatorname{PGCD}(a ; b) \mid k$.
\end{proposition}
