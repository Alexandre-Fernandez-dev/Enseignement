...
\begin{center}
  ...
  {\itshape 1\textsuperscript{ère} ST2S \quad $\bullet$ \quad Durée~: 30 minutes $\bullet$ \quad Calculatrice autorisée \quad $\bullet$ \quad \textbf{Sujet A}}
\end{center}
...
Soit $f$ la fonction définie sur $\mathbb{R}$ par $f(x) = -3x + 4$.
...
Soit $g$ la fonction définie sur $\mathbb{R}$ par $g(x) = -x^2+4$, et $\mathscr{C}_g$ sa courbe représentative dans un repère du plan.

Le point $F(-3~;~-5)$ appartient-il à $\mathscr{C}_g$~? Justifier la réponse par le calcul.
...
\begin{center}
  \vspace{-2pt}
  \includegraphics[width=6.3cm]{courbe_f.png}
  \vspace{-4pt}
\end{center}
...
\begin{enumerate}
  ...
  \item Donner un antécédent par $f$ de chacun des réels suivants~: $1$~; $3$.
