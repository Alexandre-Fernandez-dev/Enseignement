...
\tcbox[tikznode]{
Chapitre 10 \\Nombre dérivé et Fonction dérivée
}
}
...
\begin{definition}{}{}
  \begin{itemize}
    \item Pour tout $h \neq 0$ tel que $a+h \in I$, la quantité $\dfrac{f(a+h)-f(a)}{h}$ est appelée taux d'accroissement de $f$ entre $a$ et $a+h$.
    ...
    On écrit :
    $$
    \lim _{h \rightarrow 0} \frac{f(a+h)-f(a)}{h}=f^{\prime}(a) .
    $$
  \end{itemize}
\end{definition}
...
\begin{definition}{}{}
Si $f$ est dérivable en $a$, on appelle \textbf{tangente} à la courbe $\mathcal{C}_{f}$ au point d'abscisse $a$ la droite $T$ passant par le point A $(a ; f(a))$ et de coefficient directeur $f^{\prime}(a)$.
\end{definition}
...
\begin{center}
  ...
  \hline
  constante & $\mathbb{R}$ & $k$ & $\mathbb{R}$ & 0 &\\
  \hline
  identité & $\mathbb{R}$ & $x$ & $\mathbb{R}$ & 1 &\\
  \hline
  carrée & $\mathbb{R}$ & $x^{2}$ & $\mathbb{R}$ & $2 a$ &\\
  \hline
  puissance & $\mathbb{R}$ & $x^{n}$ & $\mathbb{R}$ & $n a^{n-1}$ &\\
  \hline
  inverse & $\mathbb{R}^{*}$ & $\frac{1}{x}$ & $\mathbb{R}^{*}$ & $-\dfrac{1}{a^{2}}$ &\\
  \hline
  racine carrée & $\mathbb{R}_{+}$ & $\sqrt{x}$  & $\mathbb{R}_{+}^{*}$ & $\dfrac{1}{2 \sqrt{a}}$ &\\
  \hline
  \end{tabular}
\end{center}
\end{proposition}
...
\begin{definition}{}{}
Si $f$ est dérivable sur $I$, la fonction qui a tout réel $x \in I$ associe $f^{\prime}(x)$ est appelée fonction dérivée de $f$ et elle est notée $f^{\prime}$.
\end{definition}
...
\begin{center}
  ...
  identité & $\mathbb{R}$ & $x$ & $\mathbb{R}$ & 1 &\\
  \hline
  carrée & $\mathbb{R}$ & $x^{2}$ & $\mathbb{R}$ & $2 x$ &\\
  \hline
  puissance & $\mathbb{R}$ & $x^{n}$ & $\mathbb{R}$ & $n x^{n-1}$ &\\
  \hline
  inverse & $\mathbb{R}^{*}$ & $\frac{1}{x}$ & $\mathbb{R}^{*}$ & $-\dfrac{1}{x^{2}}$ &\\[20pt]
  \hline
  racine carrée & $\mathbb{R}_{+}$ & $\sqrt{x}$ & $\mathbb{R}_{+}^{*}$ & $\dfrac{1}{2 \sqrt{x}}$ &\\
  \hline
  \end{tabular}
\end{center}
\end{proposition}
...
\begin{center}
  ...
  %Fonction & $\mathcal{D}_{f}$ & $f(x)=$ & Ensemble de dérivabilité & $f^{\prime}(x)=$ &\\
  \hline
  \makecell{Dérivée d'une somme\\ de fonctions} & $u+v$ & $(u+v)^{\prime}=u^{\prime}+v^{\prime}$ &\\
  \hline
  \makecell{Dérivée du produit d'une fonction $u$ \\ par une constante $\lambda$} & $\lambda \times u$ & $(\lambda \times u)^{\prime}=\lambda \times u^{\prime}$ & \\
  \hline
  \makecell{Dérivée d'un produit \\ de fontions} & $u \times v$ & $(uv)^{\prime}=u'v + uv'$ & \\
  \hline
  \makecell{Dérivée d'un quotient \\ de fontions} & \makecell{$\dfrac{u}{v}$ avec $v(x) \neq 0$ \\ pour tout $x \in I$} & $\left(\dfrac{u}{v}\right)'=\dfrac{u^{\prime} v-u v^{\prime}}{v^{2}}$ & \\
  \hline
  \makecell{Dérivée de l'inverse \\ d'une fonction} & \makecell{$\frac{1}{v}$ avec $v(x) \neq 0$ \\ pour tout $x \in I$} & $\left(\dfrac{1}{v}\right)'=-\dfrac{v'}{v^{2}}$ & \\
  \hline
 \end{tabular}
\end{center}
\end{proposition}
