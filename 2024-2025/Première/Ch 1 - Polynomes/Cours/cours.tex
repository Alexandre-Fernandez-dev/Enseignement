...
\begin{center}
\hide{\includegraphics[width=0.5\linewidth]{Screenshot from 2024-10-23 12-56-02.png}}
\end{center}

  \item Si $a<0$, le tableau de variations de $f$ est :

\begin{center}
\hide{\includegraphics[width=0.5\linewidth]{Screenshot from 2024-10-23 12-56-29.png}}
\end{center}
\end{itemize}
\end{proposition}
...
\hline
 & $a>0$ & $a<0$ \\
\hline
$\Delta>0$ (deux racines) & \includegraphics[max width=0.25\textwidth]{Screenshot from 2024-10-23 13-00-58.png}
 & \includegraphics[max width=0.25\textwidth]{Screenshot from 2024-10-23 13-01-08.png}
 \\
\hline
$\Delta=0$ (une racine) & \includegraphics[max width=0.25\textwidth]{Screenshot from 2024-10-23 13-01-28.png}
 & \includegraphics[max width=0.25\textwidth]{Screenshot from 2024-10-23 13-01-42.png}
 \\
\hline
$\Delta<0$ (pas de racine) & \includegraphics[max width=0.25\textwidth]{Screenshot from 2024-10-23 13-02-01.png}
...
\begin{proposition}{Calcul de $\beta = f(\alpha)$.}{}
  Soit $\beta = f(\alpha)$ le maximum (ou minimum) du polynome $f(x)$. On peut aussi calculer $\beta$ de la manière suivante:
  $$
    \beta = - \frac{\Delta}{4a}
  $$
\end{proposition}
...
\begin{definition}{Forme canonique}{}
  Soit une fonction polynomiale du second degré $f(x) = ax^2 + bx + c$ (avec $a \neq 0$). Soit $\beta = f(\alpha)$. La fonction $f$ peut toujours s'écrire sous la forme:
  $$f(x) = a(x - \alpha)^2 + \beta$$
  Cette expression est appelée la forme canonique du polynôme.
\end{definition}

\begin{proof}
  On utilise la complétion du carré avec l'identité remarquable $(a-b)^2$:
  \begin{align*}
    ax^2 + bx + c = % a\left(x^2 + \frac{b}{a} x\right) + c
    %&=
    a\left(x^2 - 2\left(-\frac{b}{2a}\right)x\right) + c
    &= a\left(x^2 - 2\alpha x\right) + c\\
    %&= a\left(x^2 - 2\alpha x + \alpha^2 - \alpha^2\right) + c\\
    &= a\left(x^2 - 2\alpha x + \alpha^2\right) - a\alpha^2 + c\\
    &= a\left(x-\alpha\right)^2 - a\alpha^2 + c\\
    %&= a\left(x-\alpha\right)^2 - a\left(-\frac{b}{2a}\right)^2 + c\\
    &= a\left(x-\alpha\right)^2 - a\left(\frac{b^2}{4a^2}\right)+ c\\
    &= a\left(x-\alpha\right)^2 - \frac{b^2}{4a} + \frac{4ac}{4a}\\
    &= a\left(x-\alpha\right)^2 - \beta\\
  \end{align*}
\end{proof}

\begin{example}
  Soit $f(x) = 3x^2 -x + 1$. On a $\alpha d= \dfrac{1}{6}$ et $\beta = \dfrac{11}{12}$ donc $f(x) = 3\left(x - \dfrac{1}{6}\right)^2 + \dfrac{11}{12}$.
\end{example}
...
\begin{itemize}
  ...
  \item Savoir utiliser la forme canonique pour lire les variations d'un polynome du second degré.
