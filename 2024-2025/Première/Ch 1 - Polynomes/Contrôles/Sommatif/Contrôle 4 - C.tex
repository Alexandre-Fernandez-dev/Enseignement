...
\noindent{\bf Contrôle 3 (C) -- 55min. \hfill Première générale \hfill 20/11/2024}
...
\begin{exercice}{(5 min)}
    Résoudre l'inéquation $4x^2 -6x + 2 \geq 0$.%suivantes :
    %Résoudre l'inéquation $2x^2 -5x + 3 \geq 0$.%suivantes :
    %\begin{tasks}(2)
    %    \task $3x^2 -5x + 2 \geq 0$. % 1 racine évidente
    %    %\task $-2x^2 -4x > 0$. % facteur évident
    %    %\task $x^2 - 6x + 9 > 0$. % ineq 1 racine
    %\end{tasks}
\end{exercice}

\begin{exercice}{(5 min)}
    Déterminer deux entiers naturels successifs dont le produit est égal à $45156$.
...
\begin{exercice}{(15 min)}
    ...
    \begin{center}
        \includegraphics[width=0.6\linewidth]{functionC.png}
...
\begin{exercice}{(10 min)}
    Donner l'unique polynôme de degré 2 passant par $A(0;\ 4), B(1;\ 4)$ et $C(-3;\ 10)$.
    %Donner l'unique polynôme de degré 2 passant par les points $A(0,4), B(1, 6)$ et $C(-2, 6)$.
\end{exercice}

\begin{exercice}{(20 min)}
Une entreprise commercialise des automobiles : chaque voiture doit être vendue 6 000 euros. Sa production $x$ peut varier entre 0 et 100 000 voitures. Suite à une étude réalisée, les coûts de production sont donnés par la formule suivante :
\[
C(x) = 50q^2 + 1\,000q + 60\,000
\]
%\[
...
\begin{enumerate}
    ...
    \item Déterminer la quantité à partir de laquelle le coût de production est supérieur à 180 000 000 €.
    ...
    \item Exprimer, en fonction de $x$, le chiffre d’affaires noté $C(x)$, en millier d’euros.
