...
\begin{solutionbox}{Solution}
Pour chaque fonction, on détermine :

- Son domaine (en tenant compte des racines de dénominateur et radicand).
- Son ensemble de dérivabilité (en général identique à son domaine car ce sont des fonctions composées de polynômes, racines, division, qui sont dérivables sur leur domaine).
- On calcule $f'(x)$ à l’aide des règles de dérivation (produit, quotient, racine) et on simplifie.

\textbf{1.} $f(x)=\sqrt{x}(x - 1)$

Domaine : $\sqrt{x}$ impose $x \geq 0$.

Dérivable : Sur $[0,+\infty[$, la racine est dérivable sur $]0,+\infty[$, mais non dérivable en 0 pour la racine (elle l’est quand même si on admet la dérivabilité sur $]0,+\infty[$). Cependant, dans un cours standard, $\sqrt{x}$ est dérivable sur $]0,+\infty[$. On retiendra donc dérivable sur $]0,+\infty[$.

$f(x)=x^{1/2}(x - 1)=x^{3/2}-x^{1/2}$.

$f'(x)=\frac{3}{2}x^{1/2}-\frac{1}{2}x^{-1/2}=\frac{3}{2}\sqrt{x}-\frac{1}{2\sqrt{x}}$ pour $x>0$.

\textbf{2.} $f(x)=\frac{1}{x -3}$
...
\textbf{3.} $f(x)=\frac{2}{3x-5}$

Domaine : $3x-5 \neq 0 \Rightarrow x \neq \frac{5}{3}$.

Dérivable sur $\mathbb{R}\setminus\{\frac{5}{3}\}$.

$f'(x)=2 \cdot \left(-\frac{3}{(3x-5)^2}\right)=-\frac{6}{(3x-5)^2}$.

\textbf{4.} $f(x)=\frac{x+1}{x+2}$

Domaine : $x \neq -2$.

Dérivable sur $\mathbb{R}\setminus\{-2\}$.

$f'(x)=\frac{(1)(x+2)-(x+1)(1)}{(x+2)^2}=\frac{x+2 - x -1}{(x+2)^2}=\frac{1}{(x+2)^2}$.

\textbf{5.} $f(x)=\frac{4}{x^2+3}$

Domaine : $x^2+3>0$ pour tout $x$, donc domaine = $\mathbb{R}$.

Dérivable sur $\mathbb{R}$.

$f'(x)=4 \cdot \frac{-2x}{(x^2+3)^2}=-\frac{8x}{(x^2+3)^2}$.

\textbf{6.} $f(x)=\left(\frac{1}{x}+1\right)x^2$

$f(x)=x^2\left(1+\frac{1}{x}\right)=x^2 + x$.

Domaine : $x \neq 0$ pour l’expression $\frac{1}{x}$, mais on simplifie $f(x)=x^2+x$ qui est défini sur tout $\mathbb{R}$. Attention, $f(x)= (1/x+1)x^2 = x+(x^2/x) = x^2 + x$ si $x\neq 0$. A $x=0$, l’expression initiale $\frac{1}{x}+1$ n’est pas définie. Donc domaine = $\mathbb{R}\setminus\{0\}$.

Dérivable : sur $\mathbb{R}\setminus\{0\}$.

$f(x)=x^2+x$, $f'(x)=2x+1$.

\textbf{7.} $f(x)=\frac{5x-1}{x+5}$

Domaine : $x \neq -5$.

Dérivable sur $\mathbb{R}\setminus\{-5\}$.

$f'(x)=\frac{5(x+5)-(5x-1)}{(x+5)^2}=\frac{5x+25-5x+1}{(x+5)^2}=\frac{26}{(x+5)^2}$.

\textbf{8.} $f(x)=\frac{\sqrt{x}}{x+1}$

Domaine : $\sqrt{x}$ impose $x\geq 0$ et $x\neq -1$ sans sens, $x+1>0$ si on veut. En fait, $x+1$ peut être négatif, ça ne pose pas de problème sauf $x \neq -1$. Mais $x\geq 0$, donc $x+1>0$. Domaine = $[0,+\infty[$.

Dérivable sur $(0,+\infty)$ (car racine dérivable sur $(0,+\infty)$).

$f(x)=x^{1/2}(x+1)^{-1}$

$f'(x)=\frac{(1/2)x^{-1/2}(x+1)-(x^{1/2}\cdot 1)}{(x+1)^2}$ (quotient rule)

Ou plus simple: appliquons quotient:
\[
f'(x)=\frac{( \tfrac{1}{2}x^{-1/2})(x+1)-\sqrt{x}\cdot 1}{(x+1)^2}=\frac{\frac{x+1}{2\sqrt{x}} - \sqrt{x}}{(x+1)^2}
=\frac{\frac{x+1 -2x}{2\sqrt{x}}}{(x+1)^2}
=\frac{1 - x}{2\sqrt{x}(x+1)^2}.
\]

\textbf{9.} $f(x)=\frac{x^3+1}{x^2+x+1}$

Domaine : le dénominateur $x^2+x+1$ est toujours >0 (discriminant <0). Domaine = $\mathbb{R}$.
...
Numérateur:
$=3x^2(x^2+x+1)-(x^3+1)(2x+1)$
$=3x^4+3x^3+3x^2-(2x^4+x^3+2x+1)$
$=(3x^4-2x^4)+(3x^3 - x^3) +3x^2 -2x -1$
$=x^4+2x^3+3x^2 -2x -1$.

$f'(x)=\frac{x^4+2x^3+3x^2 -2x -1}{(x^2+x+1)^2}$.

\textbf{10.} $f(x)=\frac{x -3}{x^2+x+1}$

Domaine : $x^2+x+1>0$ tout $x$, donc domaine = $\mathbb{R}$.

$f'(x)=\frac{(1)(x^2+x+1)-(x-3)(2x+1)}{(x^2+x+1)^2}$
...
$f'(x)=\frac{-x^2+6x+4}{(x^2+x+1)^2}$.

\textbf{11.} $f(x)=\frac{x+1}{x^2-3x-5}$

Domaine: $x^2-3x-5=(x-\frac{3}{2})^2 -\frac{29}{4}$, discriminant $9+20=29$, racines: $\frac{3\pm\sqrt{29}}{2}$. Donc $x \neq \frac{3+\sqrt{29}}{2}$ et $x \neq \frac{3-\sqrt{29}}{2}$.

Dérivable sur $\mathbb{R}\setminus\{\text{ces deux racines}\}$.

$f'(x)=\frac{(1)(x^2-3x-5)-(x+1)(2x-3)}{(x^2-3x-5)^2}$

Numérateur:
$=x^2-3x-5-(2x^2-3x+2x-3)$
$=x^2-3x-5-(2x^2 -x -3)$
$=x^2-3x-5-2x^2+x+3$
$=(-x^2 -2x -2)$.

$f'(x)=\frac{-x^2-2x-2}{(x^2 -3x -5)^2}$.

\textbf{12.} $f(x)=\frac{5}{1 - x}$

Domaine: $1-x \neq0 \Rightarrow x\neq1$.

Dérivable sur $\mathbb{R}\setminus\{1\}$.

$f'(x)=5 \cdot \frac{d}{dx}(1-x)^{-1}=5(-1)(1-x)^{-2}(-1)$

Attention, plus simple:
$f'(x)=5 \cdot \frac{d}{dx}(\frac{1}{1-x})=5\cdot\frac{0-(1-x)' }{(1-x)^2}=5 \cdot\frac{-(-1)}{(1-x)^2}=5\frac{1}{(1-x)^2}$
...
\begin{solutionbox}{Solution}
    \textbf{1.} Sur $]0,+\infty[$, la fonction $f(x)=x\sqrt{x}=x^{3/2}$ est composée de la fonction $x \mapsto x^{3/2}$, qui est dérivable pour tout $x>0$. On peut donc dériver directement :
    
    \[
    f'(x)=\frac{3}{2}x^{1/2}=\frac{3}{2}\sqrt{x}.
    \]
    
    \textbf{2.} Pour étudier la dérivabilité en $0$, on utilise la définition du nombre dérivé :
    
    \[
    f'(0)=\lim_{h \to 0}\frac{f(h)-f(0)}{h}.
    \]
    
    Or $f(0)=0\sqrt{0}=0$. Donc :
    
    \[
    f'(0)=\lim_{h \to 0}\frac{h\sqrt{h}}{h}=\lim_{h \to 0}\sqrt{h}.
    \]
    
    Quand $h \to 0^+$, $\sqrt{h} \to 0$. Donc $f'(0)=0$.
    
    \textbf{3.} La propriété du cours dit : si $f$ et $g$ sont dérivables en $a$, alors $f \times g$ est dérivable en $a$.
    
    La réciproque consisterait à dire : si $f \times g$ est dérivable en $a$, alors $f$ et $g$ sont dérivables en $a$. Cette réciproque est fausse.
    
    \underline{Justification} : Par exemple, considérons $f(x)=x$ et $g(x)=|x|$ en $0$. Le produit $f(x)g(x)=x|x|=x^2|x|/x=|x|x$. Plus concrètement, $f(x)g(x)=x|x|=x^2$ si $x>0$ et $-x^2$ si $x<0$. En fait, $x|x|=x^2$ pour tout $x$. C’est dérivable en $0$ avec dérivée $0$. Cependant, $g(x)=|x|$ n’est pas dérivable en $0$.
